...
import { profiles as profilesAPI, badges as badgesAPI } from '../services/api';
...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (banner !== (profile?.banner || '')) {
        ...
        else { toast('Bannière réservée aux VIP', 'error'); setSaving(false); return; }
      }
      if (wouaffId !== (profile?.wouaffId || '')) {
        if (!wouaffId.startsWith('@')) { setSettingsMsg({ text: "L'identifiant doit commencer par @", type: 'error' }); setSaving(false); return; }
      ...
      if (messageTheme !== ((profile as Record<string, string>)?.messageTheme || 'default')) {
        ...
        else { toast('Thème de message réservé aux VIP', 'error'); setSaving(false); return; }
      }
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      setTimeout(() => { logout(); navigate('/auth'); }, 1500);
    } catch (e: unknown) {
  ...
  const vipBadgeId = Object.entries(badgeDefs).find(([, b]) => {
  ...
  const renderThemes = () => (
    <div className="message-theme-grid">
      {THEMES.map(t => (
        <div key={t} className={`theme-option${messageTheme === t ? ' selected' : ''}`}
          onClick={() => { if (isVip) setMessageTheme(t); else toast('Thème réservé aux VIP', 'error'); }}>
  ...
  return (
    <div id="settingsPage" className="settings-page">
      <button id="mobile-nav-toggle" className="fixed top-[14px] left-[14px] z-[200] bg-[var(--bg-card2,var(--bg-card))] border border-[var(--border)] rounded-[var(--radius-sm)] text-[var(--text-primary)] w-[38px] h-[38px] flex items-center justify-center cursor-pointer shadow-[0_2px_12px_rgba(0,0,0,.3)]" onClick={() => setSidebarOpen(!sidebarOpen)} aria-label="Menu">
        <svg className="w-[18px] h-[18px]" viewBox="0 0 24 24" fill="currentColor">
          <path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z" />
        </svg>
      </button>
      <div className={`fixed inset-0 bg-black/60 z-[99] backdrop-blur-[3px]${sidebarOpen ? '' : ' hidden'}`} onClick={() => setSidebarOpen(false)} />
      ...
      <div className="settings-layout">
        <aside id="settingsSidebar" className={`settings-sidebar${sidebarOpen ? ' open' : ''}`}>
          <div className="sidebar-top">
            <div className="sidebar-brand">
              <div className="sidebar-brand-icon">
                <img src="/assets/logo/logo.png" alt="Wouaff" onError={(e) => { (e.target as HTMLElement).style.display = 'none'; }} />
              </div>
          ...
          <nav className="settings-menu">
            ...
            <div className={`settings-menu-item${activePanel === 'profile' ? ' active' : ''}`} onClick={() => { setActivePanel('profile'); setSidebarOpen(false); }}>
              <svg className="menu-icon" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" /></svg>
              Profil
            </div>
            <div className={`settings-menu-item${activePanel === 'account' ? ' active' : ''}`} onClick={() => { setActivePanel('account'); setSidebarOpen(false); }}>
              <svg className="menu-icon" viewBox="0 0 24 24"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1 1.71 0 3.1 1.39 3.1 3.1v2z" /></svg>
              Compte
            </div>
            <div className="menu-group-label mt-2">Personnalisation</div>
            <div className={`settings-menu-item${activePanel === 'badges' ? ' active' : ''}`} onClick={() => { setActivePanel('badges'); setSidebarOpen(false); }}>
              <svg className="menu-icon" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" /></svg>
              Badges
            </div>
          </nav>
          <div className="settings-footer">
            <button className="back-btn-settings" onClick={() => navigate('/')}>
              <svg className="w-[14px] h-[14px]" viewBox="0 0 24 24" fill="currentColor"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z" /></svg>
              Retour au chat
            </button>
            <button className="logout-btn-settings" onClick={logout}>
              <svg className="w-[14px] h-[14px]" viewBox="0 0 24 24" fill="currentColor"><path d="M17 7l-1.41 1.41L18.17 11H8v2h10.17l-2.58 2.58L17 17l5-5zM4 5h8V3H4c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h8v-2H4V5z" /></svg>
              Déconnexion
            </button>
          </div>
        </aside>
        ...
        <div className="settings-content">
            ...
            <div id="settings-panel-profile" className={`settings-panel${activePanel === 'profile' ? ' active' : ''}`}>
              ...
              <div className="settings-section">
                ...
                <div className="settings-item">
                  ...
                  <input id="settingsPseudo" placeholder="Votre pseudo" maxLength={32} value={pseudo} onChange={(e) => setPseudo(e.target.value)} />
                </div>
                <div className="settings-item">
                  <label htmlFor="settingsBio">Bio</label>
                  <textarea id="settingsBio" placeholder="Parlez un peu de vous..." maxLength={280} value={bio} onChange={(e) => setBio(e.target.value)} />
                ...
                <div className="settings-item">
                  ...
                  <div className="input-with-icon">
                    <input id="myWouaffId" placeholder="@votre_id" maxLength={32} value={wouaffId} onChange={(e) => setWouaffId(e.target.value)} />
                    <button className="input-copy-btn" onClick={() => { navigator.clipboard.writeText(wouaffId); toast('Identifiant copié !', 'success'); }} title="Copier">
              ...
              <div className="settings-section social-links-section">
                <div className="section-title">
                  <svg viewBox="0 0 24 24"><path d="M3.9 8.26l2.09-.36c.2-.03.38-.15.46-.33l1.36-3.02a.6.6 0 011.06-.01l1.37 3.03c.08.18.26.3.46.33l2.09.36c.48.08.66.66.33 1l-1.6 1.56c-.14.13-.2.33-.16.52l.42 2.24c.1.52-.47.9-.92.64l-1.91-1.13a.64.64 0 00-.6 0l-1.91 1.13c-.45.26-1.02-.12-.92-.64l.42-2.24a.55.55 0 00-.16-.52l-1.6-1.56c-.33-.34-.15-.92.33-1z"/><path d="M3 13h1.5v4.5c0 .28.22.5.5.5h5a.5.5 0 00.5-.5V13H12v4.5c0 1.1-.9 2-2 2H5c-1.1 0-2-.9-2-2V13z"/></svg>
                  Liens sociaux
                  <span className="vip-badge ml-2 text-[9px]">{isVip ? 'VIP' : 'Gratuit'}</span>
                </div>
                <div className="info-text mb-3.5">
                  <svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z" /></svg>
                  {isVip ? 'Jusqu\'à 3 liens sociaux. Ces liens apparaissent sur votre profil public.' : 'Version gratuite : 1 lien social. Passez VIP pour en ajouter jusqu\'à 3.'}
                </div>
                  ...
                  return (
                    <div key={i} className="settings-item social-link-row">
                      <div className="social-link-inputs">
                        ...
                        <input type="url" placeholder={pf?.id === 'other' ? 'https://...' : `URL ${pf?.label || ''}`} value={link.url}
                          onChange={(e) => {
                            const copy = [...socialLinks];
                            copy[i] = { ...copy[i], url: e.target.value };
                            setSocialLinks(copy);
                          }} className="social-url-input" />
                        <button className="social-link-remove" onClick={() => setSocialLinks(socialLinks.filter((_, j) => j !== i))}
                          title="Supprimer ce lien" aria-label="Supprimer">
                ...
                {socialLinks.length < (isVip ? 3 : 1) && (
                  <button className="social-link-add" onClick={() => {
              ...
              <div className="settings-section">
                ...
                <div className="settings-item">
                  ...
                  <input id="settingsAvatar" placeholder="https://... ou /assets/..." value={avatar} onChange={(e) => setAvatar(e.target.value)} />
                  {avatar && (
                    <div className="img-preview-chip visible avatar-preview">
                      <img src={avatar} alt="Aperçu avatar" onError={(e) => { (e.target as HTMLElement).style.display = 'none'; }} />
                ...
                <div className="settings-item">
                  ...
                  <div className="theme-toggle-row">
                    <button className={`theme-toggle-btn${theme === 'dark' ? ' active' : ''}`} onClick={() => setTheme('dark')}>
                      <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M12 3c-4.97 0-9 4.03-9 9s4.03 9 9 9 9-4.03 9-9-4.03-9-9-9zm0 16c-3.86 0-7-3.14-7-7s3.14-7 7-7 7 3.14 7 7-3.14 7-7 7z"/><path d="M12 7v10c2.76 0 5-2.24 5-5s-2.24-5-5-5z"/></svg>
                      Sombre
                    </button>
                    <button className={`theme-toggle-btn${theme === 'light' ? ' active' : ''}`} onClick={() => setTheme('light')}>
                      <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M12 7c-2.76 0-5 2.24-5 5s2.24 5 5 5 5-2.24 5-5-2.24-5-5-5zM2 13h2c.55 0 1-.45 1-1s-.45-1-1-1H2c-.55 0-1 .45-1 1s.45 1 1 1zm18 0h2c.55 0 1-.45 1-1s-.45-1-1-1h-2c-.55 0-1 .45-1 1s.45 1 1 1zM11 2v2c0 .55.45 1 1 1s1-.45 1-1V2c0-.55-.45-1-1-1s-1 .45-1 1zm0 18v2c0 .55.45 1 1 1s1-.45 1-1v-2c0-.55-.45-1-1-1s-1 .45-1 1zM5.99 4.58c-.39-.39-1.03-.39-1.41 0-.39.39-.39 1.03 0 1.41l1.06 1.06c.39.39 1.03.39 1.41 0s.39-1.03 0-1.41L5.99 4.58zm12.37 12.37c-.39-.39-1.03-.39-1.41 0-.39.39-.39 1.03 0 1.41l1.06 1.06c.39.39 1.03.39 1.41 0 .39-.39.39-1.03 0-1.41l-1.06-1.06zm1.06-10.96c.39-.39.39-1.03 0-1.41-.39-.39-1.03-.39-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06zM7.05 18.36c.39-.39.39-1.03 0-1.41-.39-.39-1.03-.39-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06z"/></svg>
                      Clair
                    </button>
                  </div>
                </div>
                <div className={`settings-item${!isVip ? ' vip-locked' : ''}`} id="bannerField">
                  ...
                  <input id="settingsBanner" placeholder="https://... ou /assets/..." value={banner} onChange={(e) => setBanner(e.target.value)} disabled={!isVip} />
                  {!isVip && (
                    <div className="vip-lock-overlay">
                      <div className="vip-lock-text">
                        <img className="vip-lock-badge" src="https://cdn-icons-png.flaticon.com/512/2583/2583344.png" alt="VIP" />
                        Réservé VIP
                      </div>
                    </div>
                  )}
              ...
              {settingsMsg && <div id="settingsMsg" className={settingsMsg.type}>{settingsMsg.text}</div>}
            </div>
              ...
              <div className="settings-section">
                ...
                <div className="settings-item">
                  ...
                  <div className="input-with-icon">
                    ...
                    <button className="input-copy-btn" onClick={() => { navigator.clipboard.writeText(user?.email || ''); toast('Email copié', 'success'); }} title="Copier">
                      <svg viewBox="0 0 24 24"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z" /></svg>
                    </button>
                  </div>
                </div>
                <div className="settings-item">
                  ...
                  <div className="input-with-icon">
                    ...
                    <button className="input-copy-btn" onClick={() => { navigator.clipboard.writeText(user?.uid || ''); toast('UID copié', 'success'); }} title="Copier">
                      <svg viewBox="0 0 24 24"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z" /></svg>
                    </button>
                  </div>
                </div>
                      ...
                      return (
                        <div key={id} className="badge-option" title={b.description || b.name || id}>
                          <div className="badge-option-icon">
                            {b.icon && <img src={b.icon} alt={b.name || id} onError={(e) => { (e.target as HTMLElement).style.display = 'none'; }} />}
                          </div>
          ...
          <aside className="settings-preview">
            ...
            <div className="profile-box">
              <div className="profile-banner" style={previewBanner ? { backgroundImage: `url(${previewBanner.replace(/"/g, '%22')})`, backgroundSize: 'cover', backgroundPosition: 'center' } : {}}></div>
              <div className="profile-avatar-wrap">
                <div className="profile-avatar-lg">
                  {previewAvatar ? <img src={previewAvatar} alt={previewPseudo} onError={(e) => { (e.target as HTMLElement).style.display = 'none'; (e.target as HTMLElement).parentElement!.textContent = initial; }} /> : <span>{initial}</span>}
                </div>
              </div>
                  ...
                  {ownedBadgeIds.map(id => {
                    ...
                    return (
                      <span key={id} className="badge-chip">
                        {b.icon && <img src={b.icon} alt="" onError={(e) => { (e.target as HTMLElement).style.display = 'none'; }} />}
                        {b.name || id}
                      </span>
                    );
                  })}
                        ...
                        {socialLinks.filter(l => l.url.trim()).map((link, i) => {
                          ...
                          return (
                            <a key={i} href={link.url} target="_blank" rel="noopener noreferrer"
                              className="preview-social-link" title={link.url}
                              style={pf ? { '--sl-color': pf.color } as React.CSSProperties : {}}>
                              {pf && <svg viewBox="0 0 24 24" fill="currentColor" className="w-[18px] h-[18px]" dangerouslySetInnerHTML={{ __html: pf.svg }} />}
      ...
      <div className={`delete-modal${deleteModalOpen ? ' visible' : ''}`} onClick={(e) => { if (e.target === e.currentTarget) setDeleteModalOpen(false); }}>
        <div className="delete-modal-box" onClick={(e) => e.stopPropagation()}>
          <h3>⚠️ Supprimer le compte</h3>
          <p>
            Cette action est <strong className="text-[var(--danger)]">irréversible</strong>.
            Tapez <strong className="text-[var(--danger)]">SUPPRIMER</strong> ci-dessous pour confirmer.
          </p>
          <input type="text" placeholder="SUPPRIMER" value={deleteConfirm}
            onChange={(e) => setDeleteConfirm(e.target.value)}
            onFocus={(e) => e.target.select()} />
          <div className="delete-modal-actions">
            <button className="cancel-btn" onClick={() => { setDeleteModalOpen(false); setDeleteConfirm(''); setDeleteError(''); }}>Annuler</button>
            <button className={`confirm-btn${deleting ? ' loading' : ''}`} disabled={deleteConfirm !== 'SUPPRIMER' || deleting} onClick={handleDeleteAccount}>
              <span className="btn-spinner"></span>
              <span className="btn-text">Supprimer</span>
            </button>
          </div>
          {deleteError && <div id="deleteError" className="visible">{deleteError}</div>}
        </div>
      </div>
