import type { GroupEntry, GroupData, UserProfile, StoryData, SearchResult } from '../types';
...
export const messages = {
  ...
  sendGroup: (gid: string, msg: Record<string, unknown>) => request<{ key: string }>('POST', `/messages/group/${gid}`, msg),
  delete: (uid: string, msgKey: string) => request<{ success: boolean }>('DELETE', `/messages/${uid}/${msgKey}`),
  deleteGroup: (gid: string, msgKey: string) => request<{ success: boolean }>('DELETE', `/messages/group/${gid}/${msgKey}`),
  update: (uid: string, msgKey: string, data: Record<string, unknown>) => request<{ success: boolean }>('PATCH', `/messages/${uid}/${msgKey}`, data),
  seen: (uid: string, msgKeys: string[]) => request<{ success: boolean }>('POST', `/messages/${uid}/seen`, { msgKeys }),
  seenGroup: (gid: string, msgKeys: string[]) => request<{ success: boolean }>('POST', `/messages/group/${gid}/seen`, { msgKeys }),
  search: (uid: string, q: string) => request<{ results: Record<string, unknown> }>('GET', `/messages/search/${uid}?q=${encodeURIComponent(q)}`),
  searchGroup: (gid: string, q: string) => request<{ results: Record<string, unknown> }>('GET', `/messages/group/search/${gid}?q=${encodeURIComponent(q)}`),
  pin: (uid: string, msgKey: string, pinned: boolean) => request<{ success: boolean }>('POST', `/messages/${uid}/${msgKey}/pin`, { pinned }),
  pinGroup: (gid: string, msgKey: string, pinned: boolean) => request<{ success: boolean }>('POST', `/messages/group/${gid}/${msgKey}/pin`, { pinned }),
...
export const profiles = {
  ...
  getPublicKey: (uid: string) => request<{ publicKey: Record<string, unknown> | null }>('GET', `/profiles/${uid}/publicKey`),
...
export const groups = {
  ...
  update: (gid: string, data: Record<string, unknown>) =>
    request<{ success: boolean }>('PUT', `/groups/${gid}`, data),
  ...
  removeMember: (gid: string, uid: string) =>
    request<{ success: boolean }>('DELETE', `/groups/${gid}/members/${uid}`),
  setRole: (gid: string, uid: string, role: string) =>
    request<{ success: boolean }>('PUT', `/groups/${gid}/members/${uid}/role`, { role }),
  newInvite: (gid: string) =>
    request<{ inviteId: string }>('POST', `/groups/${gid}/invite`),
  join: (inviteId: string) =>
    request<{ success?: boolean; alreadyMember?: boolean; gid: string }>('POST', `/groups/join/${inviteId}`),
  public: () =>
    request<Record<string, unknown>[]>('GET', '/groups/public'),
  report: (gid: string) =>
    request<{ success: boolean }>('POST', `/groups/${gid}/report`),
};

/* ── Contacts ── */
export const contacts = {
  list: () => request<Record<string, UserProfile>>('GET', '/contacts'),
  add: (wouaffId: string) => request<{ uid: string; profile?: UserProfile; requested?: boolean; autoAccepted?: boolean }>('POST', '/contacts', { wouaffId }),
...
export const stories = {
  ...
  create: (media: string, type?: string, audioData?: string, audioName?: string, audioStartTime?: number, audioExtractDuration?: number, description?: string) =>
    request<{ storyId: string } & StoryData>('POST', '/stories', { media, type: type || 'image', audioData, audioName, audioStartTime, audioExtractDuration, description }),
...
export const search = {
  ...
  userByWouaffId: (wouaffId: string) => request<{ uid: string; profile: UserProfile }>('GET', `/search/users/${wouaffId.replace('@', '')}`),
};
...
export const admin = {
  ...
  badges: {
    ...
    addToUser: (uid: string, badgeId: string) => request<{ success: boolean }>('POST', `/admin/badges/${uid}/add/${badgeId}`),
    set: (uid: string, badgeIds: string[]) => request<{ success: boolean }>('PUT', `/admin/badges/${uid}`, { badgeIds }),
  ...
  profile: {
    update: (uid: string, data: Record<string, unknown>) => request<{ success: boolean }>('PUT', `/admin/profile/${uid}`, data),
