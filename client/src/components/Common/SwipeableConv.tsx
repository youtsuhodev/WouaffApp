  ...
  return (
    <div className="relative overflow-hidden" onClick={reset}>
      <div className={`absolute inset-y-0 right-0 flex${swiped ? '' : ' hidden'}`}>
        {actions.map((a, i) => (
          <button key={i} className={`flex flex-col items-center justify-center w-[70px] text-xs font-bold border-none cursor-pointer text-white ${a.type === 'danger' ? 'bg-red-500' : 'bg-blue-500'}`} onClick={(e) => { e.stopPropagation(); a.onClick(); setSwiped(false); }}>
            <svg className="w-5 h-5 mb-1" viewBox="0 0 24 24" dangerouslySetInnerHTML={{ __html: a.icon }} />
            <span>{a.label}</span>
          </button>
        ))}
      </div>
      <div ref={itemRef} className={`relative z-10 transition-transform duration-200${swiped ? ' -translate-x-[70px]' : ''}`}
