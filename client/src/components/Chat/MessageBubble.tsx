import { renderLinkifiedText, renderLinkPreviews, formatTime, groupReactions, toggleReaction } from '../../utils/chatHelpers';
...
export default function MessageBubble({ mid, msg, isSent, isGroup, groupClass, prevSame, profiles, allMessages, linkPreviews, user, convId, currentGroupId, onOpenUserProfile, onContextMenu, onTouchStart, onTouchEnd, onTouchMove, onDownloadFile }: MessageBubbleProps) {
  ...
  return (
    <div className={`msg-wrapper ${isSent ? 'sent' : 'recv'} ${groupClass || ''}`} data-mid={mid}>
      ...
      >
        ...
        {msg.forwardedFrom && (
          <div className="msg-forwarded">Transféré</div>
        )}
        {msg.deleted ? (
          <div className="msg-text deleted">🚫 Message supprimé</div>
        ) : msg.type === 'image' ? (
          <div className="msg-image">
            <img src={msg.imageData || msg.text} alt="Image" loading="lazy"
              onClick={() => window.open(msg.imageData || msg.text)} className="cursor-pointer max-w-full max-h-[300px] rounded-lg block"
            />
          </div>
        ) : msg.type === 'file' ? (
          <div className="msg-file">
            <div className="file-link" onClick={() => {
              ...
              <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm-3.06 16L7.4 14.46l1.41-1.41 2.12 2.12 4.24-4.24 1.41 1.41L10.94 18z"/></svg>
        ...
        <div className="msg-footer">
          ...
          {!!msg.ephemeralDuration && <span className="msg-ephemeral" title="Message éphémère">🕐</span>}
          {!!msg.edited && <span className="msg-edited">modifié</span>}
          <span className="msg-time">{formatTime(msg.time)}</span>
          {isSent && (currentGroupId ? (
            msg.seenBy && msg.seenBy.length > 0 ? (
              <span className="msg-group-seen" title={`Vu par ${msg.seenBy.length} personne${msg.seenBy.length > 1 ? 's' : ''}`}>
            ...
            <span className={`msg-status${msg.seen ? ' seen' : ''}`}>
              <svg className="check1" width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" /></svg>
              {!!msg.seen && <svg className="check2" width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" /></svg>}
            </span>
          ))}
        </div>
        {msg.reactions && (
          <div className="msg-reactions" data-mid={mid}>
            {Object.entries(groupReactions(msg.reactions)).map(([emoji, uids]) => (
              <div key={emoji} className={`reaction-chip${uids.includes(user?.uid || '') ? ' mine' : ''}`}
                data-emoji={emoji} data-mid={mid}
