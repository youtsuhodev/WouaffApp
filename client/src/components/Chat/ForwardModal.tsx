...
export default function ForwardModal({ msg, user, currentChatWith, currentGroupId, onClose }: ForwardModalProps) {
  ...
  useEffect(() => {
    contactsAPI.list().then(r => setContacts(r as any)).catch(() => {});
    groupsAPI.list().then(r => setGroups(r as any)).catch(() => {});
  }, []);
  ...
  const doForward = async (targetUid?: string, targetGid?: string) => {
    ...
    if (msg.imageData) { payload.imageData = msg.imageData; payload.type = 'image'; }
    if (msg.fileData) { payload.fileData = msg.fileData; payload.fileName = msg.fileName; payload.type = 'file'; }
    if (msg.audioData) { payload.audioData = msg.audioData; payload.duration = msg.duration; payload.type = 'voice'; }
    if (msg.ephemeralDuration) { payload.ephemeralDuration = msg.ephemeralDuration; }
    try {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/60 flex items-center justify-center" onClick={onClose}>
      <div className="bg-[var(--bg-card)] rounded-2xl p-5 max-w-[360px] w-[90%] max-h-[70vh] overflow-y-auto border border-[var(--border)]" onClick={e => e.stopPropagation()}>
        <h3 className="text-base font-bold m-0 mb-3 text-[var(--text-primary)]">Transférer le message</h3>
        {Object.keys(contacts).length === 0 && Object.keys(groups).length === 0 && <p className="text-text-muted text-xs">Aucune conversation disponible</p>}
            ...
            {Object.entries(groups).map(([gid, g]) => {
              ...
              return (
                <div key={gid} className="forward-item" onClick={() => doForward(undefined, gid)}>
                  <div className="forward-item-avatar">{g.group.icon ? <img src={g.group.icon} alt="" /> : <span>{(g.group.name || 'G')[0]}</span>}</div>
            ...
            {Object.entries(contacts).map(([uid, c]) => {
              ...
              return (
                <div key={uid} className="forward-item" onClick={() => doForward(uid)}>
                  <div className="forward-item-avatar">{c.avatar ? <img src={c.avatar} alt="" /> : <span>{(c.pseudo || '?')[0]}</span>}</div>
