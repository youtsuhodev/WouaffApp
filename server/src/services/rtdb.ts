...
export async function getConversationsForUser(uid: string): Promise<Record<string, unknown>> {
  const contactRows = await query<Array<{ contactUid: string }>>('SELECT contactUid FROM contacts WHERE uid = ?', [uid]);
  ...
  const entries = await Promise.all(
    contactRows.map(async (row) => {
      ...
      const msgRows = await query<Array<MessageData & { convId: string }>>(
        'SELECT * FROM messages WHERE convId = ? ORDER BY time DESC LIMIT 1', [chatId(uid, contactUid)]
      );
  ...
  const entries = await Promise.all(
    memberRows.map(async (row) => {
      ...
      const msgRows = await query<Array<MessageData & { gid: string }>>(
        'SELECT * FROM group_messages WHERE gid = ? ORDER BY time DESC LIMIT 1', [row.gid]
      );
      const lastMsg = msgRows.length > 0 ? msgRows[0] : null;
...
export async function getMessages(convId: string, limit: number = 50, before?: number): Promise<{ messages: Record<string, MessageData>; hasMore: boolean }> {
  const take = limit + 1;
...
export async function pushMessage(convId: string, msg: MessageData): Promise<string> {
  ...
  await query(
    ...
    [convId, key, msg.from, msg.text || null, msg.type || 'text', msg.time || Date.now(), 0,
     msg.encrypted ? 1 : 0, msg.ct || null, msg.iv || null, msg.imageData || null,
     msg.fileData || null, msg.fileName || null, msg.audioData || null, msg.duration || null,
     msg.contact ? JSON.stringify(msg.contact) : null, msg.pendingFrom || null, msg.senderName || null,
     msg.replyTo || null, msg.messageTheme || null, msg.forwardedFrom || null,
...
export async function updateMessage(convId: string, msgKey: string, updates: Partial<MessageData>): Promise<void> {
  ...
  if (updates.edited !== undefined) { fields.push('edited=?'); params.push(updates.edited ? 1 : 0); }
  if (updates.deleted !== undefined) { fields.push('deleted=?'); params.push(updates.deleted ? 1 : 0); }
  if (updates.pinned !== undefined) { fields.push('pinned=?'); params.push(updates.pinned ? 1 : 0); }
  if (updates.seen !== undefined) { fields.push('seen=?'); params.push(updates.seen); }
  if (updates.reactions !== undefined) { fields.push('reactions=?'); params.push(JSON.stringify(updates.reactions)); }
...
export async function markMessagesAsSeen(convId: string, msgKeys: string[], timestamp: number): Promise<void> {
  ...
  await query(`UPDATE messages SET seen=? WHERE convId=? AND msgKey IN (${placeholders})`, [timestamp, convId, ...msgKeys]);
}

/* ── Group Messages ── */

export async function getGroupMessages(gid: string, limit: number = 50, before?: number): Promise<{ messages: Record<string, MessageData>; hasMore: boolean }> {
  const take = limit + 1;
  const rows = await query<Array<MessageData & { msgKey: string }>>(
    before
  ...
  for (const row of rows) {
    ...
    if (msg.seenBy && typeof msg.seenBy === 'string') {
      try { msg.seenBy = JSON.parse(msg.seenBy as string); } catch { msg.seenBy = []; }
    }
    result[key] = msg;
  }
  return { messages: result, hasMore };
...
export async function pushGroupMessage(gid: string, msg: MessageData): Promise<string> {
  ...
  await query(
    ...
    [gid, key, msg.from, msg.text || null, msg.type || 'text', msg.time || Date.now(),
     0, 0, msg.encrypted ? 1 : 0, msg.ct || null, msg.iv || null, msg.imageData || null,
     msg.fileData || null, msg.fileName || null, msg.audioData || null, msg.duration || null,
     msg.senderName || null, msg.replyTo || null, msg.messageTheme || null, msg.forwardedFrom || null,
     msg.seenBy ? JSON.stringify(msg.seenBy) : null, msg.ephemeralDuration || null]
  );
  return key;
}
...
export async function updateGroupMessage(gid: string, msgKey: string, updates: Partial<MessageData>): Promise<void> {
  ...
  if (updates.pinned !== undefined) { fields.push('pinned=?'); params.push(updates.pinned ? 1 : 0); }
  if (updates.reactions !== undefined) { fields.push('reactions=?'); params.push(JSON.stringify(updates.reactions)); }
  if (updates.seenBy !== undefined) { fields.push('seenBy=?'); params.push(JSON.stringify(updates.seenBy)); }
  if (fields.length === 0) return;
...
export async function getProfile(uid: string): Promise<Record<string, unknown> | null> {
  ...
  if (publicKey) {
    try { result.publicKey = JSON.parse(publicKey as string); } catch { result.publicKey = publicKey; }
  }
  const badgeRows = await query<Array<{ badgeId: string }>>('SELECT badgeId FROM user_badges WHERE uid=? ORDER BY sortOrder ASC', [uid]);
...
const PROFILE_COLUMNS = new Set([
  'pseudo', 'bio', 'email', 'passwordHash', 'avatar', 'banner',
  'wouaffId', 'publicKey', 'status', 'lastSeen', 'createdAt', 'social_links'
]);
...
export async function updateProfile(uid: string, data: Record<string, unknown>): Promise<void> {
  if (data.wouaffId !== undefined) {
    ...
    if (newId !== oldId) {
      ...
      if (newId) await query('INSERT INTO wouaff_id_index (wouaffId, uid) VALUES (?,?) ON DUPLICATE KEY UPDATE uid=VALUES(uid)', [newId, uid]);
    }
  }
  const fields: string[] = [];
  const params: any[] = [];
...
export async function getPublicKey(uid: string): Promise<Record<string, unknown> | null> {
  ...
  try { return JSON.parse(row.publicKey); } catch { return null; }
}
...
export async function getGroup(gid: string): Promise<Record<string, unknown> | null> {
  ...
  const members = await query<Array<{ uid: string; role: string; joinedAt: number }>>(
    'SELECT uid, role, joinedAt FROM group_members WHERE gid = ?', [gid]
  );
...
export async function getPublicGroups(): Promise<Record<string, unknown>[]> {
  ...
  for (const row of rows) {
    const members = await query<Array<{ uid: string; role: string }>>(
      'SELECT uid, role FROM group_members WHERE gid = ?', [row.gid]
    );
  ...
  if (members) {
    for (const [uid, info] of Object.entries(members)) {
      await query('INSERT INTO group_members (gid, uid, role, joinedAt) VALUES (?,?,?,?)',
        [gid, uid, info.role || 'member', info.joinedAt || Date.now()]);
    }
  }
  return gid;
}
...
export async function addGroupMember(gid: string, uid: string, role: string = 'member'): Promise<void> {
  await query('INSERT INTO group_members (gid, uid, role, joinedAt) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE role=VALUES(role), joinedAt=VALUES(joinedAt)',
...
export async function createGroupInvite(inviteId: string, groupId: string): Promise<void> {
  await query('INSERT INTO group_invites (inviteId, gid, createdAt) VALUES (?,?,?) ON DUPLICATE KEY UPDATE createdAt=VALUES(createdAt)',
...
export async function reportGroup(gid: string, name: string, reporterUid: string): Promise<void> {
  await query('UPDATE groups_table SET reported=1, reportedBy=?, reportedAt=? WHERE gid=?', [reporterUid, Date.now(), gid]);
}
...
export async function addContact(uid: string, contactUid: string): Promise<void> {
  await query('INSERT INTO contacts (uid, contactUid, addedAt) VALUES (?,?,?) ON DUPLICATE KEY UPDATE addedAt=VALUES(addedAt)',
...
export async function searchByWouaffId(wouaffId: string): Promise<string | null> {
  ...
  const profile = await getOne<{ uid: string; wouaffId: string | null }>('SELECT uid, wouaffId FROM profiles WHERE wouaffId=?', [wouaffId]);
  if (profile?.uid) {
    await query('INSERT INTO wouaff_id_index (wouaffId, uid) VALUES (?,?) ON DUPLICATE KEY UPDATE uid=VALUES(uid)', [wouaffId, profile.uid]);
    return profile.uid;
  }
  return null;
}
...
export async function getStories(uid: string): Promise<Record<string, unknown>> {
  const rows = await query<Array<Record<string, unknown>>>(
    'SELECT * FROM stories WHERE uid=? AND expiresAt>? ORDER BY createdAt DESC', [uid, Date.now()]
  );
  const result: Record<string, unknown> = {};
  for (const row of rows) {
    const storyId = (row as any).storyId as string;
    const views = await query<Array<{ viewedBy: string }>>('SELECT viewedBy FROM story_views WHERE storyId=?', [storyId]);
...
export async function createStory(uid: string, storyData: Record<string, unknown>): Promise<string> {
  ...
  const audioExtractDuration = typeof storyData.audioExtractDuration === 'number' ? storyData.audioExtractDuration : null;
  ...
  await query(
    ...
    [uid, storyId, media, type, now, now + 86400000, audioData, audioName, audioStartTime, audioExtractDuration, description]
  );
  return storyId;
}

export async function markStoryViewed(uid: string, storyId: string, viewerUid: string): Promise<void> {
  await query('INSERT INTO story_views (storyId, viewedBy, viewedAt) VALUES (?,?,?) ON DUPLICATE KEY UPDATE viewedAt=VALUES(viewedAt)',
...
export async function cleanupExpiredStories(uid: string): Promise<void> {
  const expired = await query<Array<{ storyId: string }>>('SELECT storyId FROM stories WHERE uid=? AND expiresAt<=?', [uid, Date.now()]);
...
export async function setFcmToken(uid: string, token: string): Promise<void> {
  await query('INSERT INTO fcm_tokens (uid, token, createdAt) VALUES (?,?,?) ON DUPLICATE KEY UPDATE createdAt=VALUES(createdAt)',
    [uid, token, Date.now()]);
}
...
export async function addBadgeToUser(uid: string, badgeId: string): Promise<void> {
  await query('INSERT INTO user_badges (uid, badgeId) VALUES (?,?) ON DUPLICATE KEY UPDATE sortOrder=sortOrder', [uid, badgeId]);
}
...
export async function setDeletedConversation(uid: string, convId: string): Promise<void> {
  await query('INSERT INTO deleted_convs (uid, convId, deletedAt) VALUES (?,?,?) ON DUPLICATE KEY UPDATE deletedAt=VALUES(deletedAt)',
    [uid, convId, Date.now()]);
}
...
export async function setStaff(uid: string, isStaffMember: boolean): Promise<void> {
  if (isStaffMember) {
    await query('INSERT INTO staff (uid, addedAt) VALUES (?,?) ON DUPLICATE KEY UPDATE addedAt=VALUES(addedAt)', [uid, Date.now()]);
  } else {
...
export async function getAdminStats(): Promise<{
  users: number; chats: number; messages: number; online: number; badges: number; wouaffIds: number;
}> {
...
export async function getRecentUsers(limit = 20): Promise<Record<string, Record<string, unknown>>> {
  const rows = await query<Array<{ uid: string } & Record<string, unknown>>>(
    'SELECT * FROM profiles ORDER BY createdAt DESC LIMIT ?', [limit]
  );
...
export async function updateProfileByAdmin(uid: string, data: Record<string, unknown>): Promise<void> {
  ...
  if (data.wouaffId !== undefined) { fields.push('wouaffId=?'); params.push(data.wouaffId || null); }
  if (fields.length === 0) return;
  params.push(uid);
  await query(`UPDATE profiles SET ${fields.join(',')} WHERE uid=?`, params);
}
...
export async function acceptContactRequest(uid: string, fromUid: string): Promise<boolean> {
  const row = await getOne<{ id: number }>(
    'SELECT id FROM contact_requests WHERE fromUid=? AND toUid=? AND status=?', [fromUid, uid, 'pending'],
  );
  if (!row) return false;
...
export async function getPendingRequests(uid: string): Promise<Array<{ fromUid: string; createdAt: number }>> {
  return query<Array<{ fromUid: string; createdAt: number }>>(
    'SELECT fromUid, createdAt FROM contact_requests WHERE toUid=? AND status=? ORDER BY createdAt DESC', [uid, 'pending'],
  );
}
...
export async function getSentRequests(uid: string): Promise<Array<{ toUid: string; createdAt: number }>> {
  return query<Array<{ toUid: string; createdAt: number }>>(
    'SELECT toUid, createdAt FROM contact_requests WHERE fromUid=? AND status=? ORDER BY createdAt DESC', [uid, 'pending'],
  );
}
...
export async function blockUser(uid: string, blockedUid: string): Promise<void> {
  await query('INSERT INTO blocks (uid, blockedUid, blockedAt) VALUES (?,?,?) ON DUPLICATE KEY UPDATE blockedAt=VALUES(blockedAt)',
...
export async function isBlocked(uid: string, blockedUid: string): Promise<boolean> {
  const row = await getOne<{ blockedUid: string }>('SELECT blockedUid FROM blocks WHERE uid=? AND blockedUid=?', [uid, blockedUid]);
  return !!row;
}
...
export async function getAdminLogs(limit = 50): Promise<Array<{
  id: number; adminUid: string; action: string; targetType: string | null;
  targetId: string | null; details: string | null; createdAt: number;
}>> {
  return query(
    'SELECT * FROM admin_logs ORDER BY createdAt DESC LIMIT ?', [limit],
  );
}

export async function getReportedGroups(): Promise<Array<{
...
export async function cleanExpiredEphemeralMessages(): Promise<Array<{ type: 'dm' | 'group'; convId: string; key: string }>> {
  const now = Date.now();
