...
import type { AuthRequest, VideoData, VideoComment } from '../types/index.js';
...
router.post('/', upload.fields([{ name: 'video', maxCount: 1 }]), async (req: Request, res: Response) => {
  ...
  if (!videoFile) { res.status(400).json({ error: 'Vidéo requise' }); return; }
  ...
  try {
    ...
    const location = lat && lng ? { lat: parseFloat(lat), lng: parseFloat(lng), name: locationName || undefined } : null;
    const now = Date.now();
    await query(
...
router.get('/', async (req: Request, res: Response) => {
  ...
  const rows = await query<Array<VideoData & { uid: string }>>(
    'SELECT * FROM videos ORDER BY createdAt DESC LIMIT ? OFFSET ?', [limit, offset]
  );
  ...
  for (const row of rows) {
    ...
    let liked = false;
    if (authReq.uid) {
      const likeRow = await getOne<{ uid: string }>('SELECT uid FROM video_likes WHERE uid=? AND videoId=?', [authReq.uid, row.id]);
      liked = !!likeRow;
    }
    let location = row.location;
    if (typeof location === 'string') { try { location = JSON.parse(location); } catch { location = null; } }
    enriched.push({
      ...row,
...
router.get('/:id', async (req: Request, res: Response) => {
  ...
  if (!row) { res.status(404).json({ error: 'Vidéo introuvable' }); return; }
  const profile = await getProfile(row.uid);
  let liked = false;
  if (authReq.uid) {
    const likeRow = await getOne<{ uid: string }>('SELECT uid FROM video_likes WHERE uid=? AND videoId=?', [authReq.uid, row.id]);
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  if (!row) { res.status(404).json({ error: 'Vidéo introuvable' }); return; }
  if (row.uid !== authReq.uid) { res.status(403).json({ error: 'Interdit' }); return; }
...
router.post('/:id/like', async (req: Request, res: Response) => {
  const authReq = req as AuthRequest;
  const existing = await getOne<{ uid: string }>('SELECT uid FROM video_likes WHERE uid=? AND videoId=?', [authReq.uid!, req.params.id]);
  if (existing) {
    ...
    await query('INSERT INTO video_likes (uid, videoId, createdAt) VALUES (?,?,?)', [authReq.uid!, req.params.id, Date.now()]);
...
router.post('/:id/comments', async (req: Request, res: Response) => {
  ...
  if (!text || !text.trim()) { res.status(400).json({ error: 'Texte requis' }); return; }
  const video = await getOne<{ id: string }>('SELECT id FROM videos WHERE id=?', [req.params.id]);
  if (!video) { res.status(404).json({ error: 'Vidéo introuvable' }); return; }
  const now = Date.now();
  const result = await query<{ insertId: number }>('INSERT INTO video_comments (videoId, uid, text, createdAt) VALUES (?,?,?,?)', [req.params.id, authReq.uid!, text.trim(), now]);
...
router.get('/:id/comments', async (req: Request, res: Response) => {
  const rows = await query<Array<VideoComment & { uid: string }>>(
    'SELECT * FROM video_comments WHERE videoId=? ORDER BY createdAt ASC', [req.params.id]
  );
...
router.delete('/comments/:commentId', async (req: Request, res: Response) => {
  const authReq = req as AuthRequest;
  const comment = await getOne<{ uid: string; videoId: string }>('SELECT uid, videoId FROM video_comments WHERE id=?', [req.params.commentId]);
  if (!comment) { res.status(404).json({ error: 'Commentaire introuvable' }); return; }
  if (comment.uid !== authReq.uid) { res.status(403).json({ error: 'Interdit' }); return; }
